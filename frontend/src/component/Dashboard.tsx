...
export default function Dashboard() {
  ...
  const fetchTasks = async () => {
    try {
      const res = await axios.get('https://taskmanager2-10.onrender.com/api/tasks', {
  ...
  const fetchTaskById = async (id: string) => {
    try {
      const res = await axios.get(`https://taskmanager2-10.onrender.com/api/tasks/${id}`, {
        headers: { Authorization: `Bearer ${token}` },
      });
  ...
  const handleUpdateTask = async (updatedTask: Task) => {
    try {
      const res = await axios.put(
        `https://taskmanager2-10.onrender.com/api/tasks/${updatedTask._id}`,
        updatedTask,
        {
          headers: { Authorization: `Bearer ${token}` },
        }
  ...
  const handleDeleteTask = async (id: string) => {
    ...
    try {
      await axios.delete(`https://taskmanager2-10.onrender.com/api/tasks/${id}`, {
        headers: { Authorization: `Bearer ${token}` },
      });
